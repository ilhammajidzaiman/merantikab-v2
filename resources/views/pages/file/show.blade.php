{{-- <x-public.layout.app-layout title="{{ $record->title ?? null }}">
                    ...
                    @if ($record->slug)
                        <div class="col-span-full md:col-span-4">
                            <div class="flex space-x-2 items-center font-normal text-md text-slate-600 mb-4">
                                <a wire:navigate href="{{ route('file.index') }}"class="hover:underline">
                                    Dokumen
                                </a>
                                ...
                                <a wire:navigate
                                    href="{{ route('file.index') }} "class="hover:underline text-emerald-500 line-clamp-1">
                                    {{ $record->title ?? null }}
                                </a>
                            </div>
                            <h1 class="font-bold text-xl text-slate-600 mb-4">
                                {{ $record->title ?? null }}
                            </h1>
                            ...
                            @if ($record->image === null)
                                <div
                                    class="aspect-video w-auto h-auto flex items-center justify-center overflow-hidden rounded-xl shrink-0 bg-slate-200">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            ...
                            @endif
                            <div class="content font-normal text-lg text-slate-600 mt-4 ">
                                {!! $record->content ?? null !!}
                            </div>
                            ...
                            <section class="grid grid-cols-1 gap-4">
                                @if ($record->attachment)
                                    <embed type="application/pdf"
                                        src="{{ env('API_ADMIN') . $record->attachment ?? null }}"
                                        class="w-full min-h-screen rounded-xl shadow-md"></embed>
                                @endif
                            </section>
                        </div>
                    @else
    ...
    @if ($record->slug)
        @push('metaTag')
            @if ($record)
                <meta property="og:url" content="{{ route('file.show', $item->slug ?? null) }}">
                <meta property="og:type" content="Publikasi Dokumen">
                ...
                <meta property="twitter:domain" content="{{ route('file.show', $item->slug ?? null) }}">
                <meta property="twitter:url" content="{{ route('file.show', $item->slug ?? null) }}">
        ...
        @push('scripts')
            <script>
                const url = "{{ route('file.show', $item->slug ?? null) }}";
                    ...
                    <div class="col-span-full space-y-4">
                        @if ($record)
                            ...
                            @if ($record->attachment)
                                <embed type="application/pdf"
                                    src="{{ env('API_ADMIN') . $record->attachment ?? null }}"
                                    class="aspect-video w-full rounded-xl shadow"></embed>
                            @endif
                        @else
                            <x-empty />
                        @endif
